...
export default function WritingDetail() {
  ...
  const relatedWritings = writings.filter(w => w.id !== id && w.category === writing?.category).slice(0, 3);
    ...
    return (
      <div className="fixed inset-0 z-50 bg-background overflow-auto">
        ...
        <div className="max-w-3xl mx-auto py-20 px-8">
          <h1 className="font-serif text-4xl md:text-5xl font-semibold mb-8 text-center">{writing.title}</h1>
    ...
    <div className="container py-12 animate-fade-in">
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          ...
          <Button variant="outline" size="sm" onClick={() => setStageMode(true)}>
        ...
        <article>
          ...
          <h1 className="font-serif text-4xl md:text-5xl font-semibold mb-4">{writing.title}</h1>
        ...
        {relatedWritings.length > 0 && (
          <section className="mt-16 pt-8 border-t">
            <h2 className="font-serif text-2xl font-semibold mb-6">More {writing.category}</h2>
            <div className="grid md:grid-cols-3 gap-4">
              {relatedWritings.map(related => (
                <Link key={related.id} to={`/writing/${related.id}`}>
                  <Card className="card-hover h-full">
                    <CardContent className="p-4">
                      <h3 className="font-serif font-semibold mb-2">{related.title}</h3>
